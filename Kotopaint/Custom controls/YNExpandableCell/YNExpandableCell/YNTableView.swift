...
open class YNTableView: UITableView, UITableViewDataSource, UITableViewDelegate {
    ...
    public func tableView(_ tableView: UITableView, willSelectRowAt indexPath: IndexPath) -> IndexPath? {
        if let cell = tableView.cellForRow(at: indexPath) {
            if let view = tableView as? YNTableView {
                view.collapseAll()
            }
            let pointInTable = cell.convert(cell.bounds.origin, to: tableView)
            let newIndexPath = tableView.indexPathForRow(at: pointInTable)
            return newIndexPath
        }
